    ...
    D0: Into<SDMMC::D0>,
{
    ...
    type AltPins = (SDMMC::Ck, SDMMC::Cmd, SDMMC::D0);
    fn convert(self) -> Self::AltPins {
...
pub trait SdmmcExt<P: SdmmcPeripheral>: Sized + Instance {
...
impl<SDMMC: Instance, P: SdmmcPeripheral> SdmmcExt<P> for SDMMC {
    fn sdmmc<PINS>(
        self,
        pins: PINS,
        prec: Self::Rec,
        ...
        PINS: Pins<Self>,
    {
        ...
        Sdmmc::new(self, PINS::BUSWIDTH, prec, clocks)
    }
    ...
    fn sdmmc_unchecked(
        ...
        Sdmmc::new(self, bus_width, prec, clocks)
    }
}

impl<SDMMC: Instance, P: SdmmcPeripheral> Sdmmc<SDMMC, P> {
    ...
    pub fn new(
        ...
        let ker_ck = match prec.get_kernel_clk_mux() {
            rec::SdmmcClkSel::Pll1Q => clocks.pll1_q_ck().expect(concat!(
                stringify!($SDMMCX),
                ": PLL1_Q must be enabled"
            )),
            rec::SdmmcClkSel::Pll2R => clocks.pll2_r_ck().expect(concat!(
                stringify!($SDMMCX),
                ": PLL2_R must be enabled"
            )),
        };
        ...
        sdmmc.clkcr.write(|w| unsafe {
            w.widbus().bits(0); // 1-bit wide bus
            w.clkdiv().bits(clkdiv);
            w.pwrsav().clear_bit(); // power saving
            w.negedge().clear_bit(); // dephasing selection bit
            w.hwfc_en().set_bit() // hardware flow control
        });
    ...
    fn start_datapath_transfer(
        ...
        self.sdmmc.dctrl.write(|w| unsafe {
            w.dblocksize().bits(block_size); // 2^n bytes block size
            w.dtdir().bit(dtdir);
            w.dten().set_bit() // Enable transfer
        });
    }
    ...
    fn cmd<R: common_cmd::Resp>(&self, cmd: Cmd<R>) -> Result<(), Error> {
        ...
        self.sdmmc.icr.modify(|_, w| {
            w.ccrcfailc() // CRC FAIL
                .set_bit();
            w.ctimeoutc().set_bit(); // TIMEOUT
            w.cmdrendc().set_bit(); // CMD R END
            w.cmdsentc().set_bit(); // CMD SENT
            w.dataendc().set_bit();
            w.dbckendc().set_bit();
            w.dcrcfailc().set_bit();
            w.dtimeoutc().set_bit();
            w.sdioitc().set_bit(); // SDIO IT
            w.rxoverrc().set_bit();
            w.txunderrc().set_bit()
        });
        ...
        self.sdmmc.cmdr.write(|w| unsafe {
            w.waitint().clear_bit();
            w.waitresp().bits(waitresp); // No / Short / Long
            w.cmdstop().bit(cpsm_stop_transmission); // CPSM Stop Transmission
            w.cmdindex().bits(cmd.cmd);
            w.cpsmen().set_bit()
        });
...
impl<SDMMC: Instance> Sdmmc<SDMMC, SdCard> {
    ...
    pub fn init(&mut self, freq: impl Into<Hertz>) -> Result<(), Error> {
        ...
        if freq.raw() > 25_000_000 {
            // Switch to SDR25
            self.signaling =
                self.switch_signaling_mode(SdCardSignaling::SDR25)?;
    ...
    fn read_sd_status(&mut self) -> Result<(), Error> {
        ...
        while {
            ...
            if sta_reg.rxfifohf().bit() {
                for _ in 0..8 {
                    status[15 - idx] =
                        u32::from_be(self.sdmmc.fifor.read().bits());
    ...
    pub fn sdmmc_block_device(self) -> SdmmcBlockDevice<Sdmmc<SDMMC, SdCard>> {
        SdmmcBlockDevice {
...
impl<SDMMC: Instance> embedded_sdmmc::BlockDevice
    for SdmmcBlockDevice<Sdmmc<SDMMC, SdCard>>
{
    type Error = Error;
...
impl<SDMMC: Instance> Sdmmc<SDMMC, Emmc> {
    ...
    pub fn read_extended_csd(&mut self) -> Result<ExtCSD, Error> {
        ...
        while {
            ...
            if sta_reg.rxfifohf().bit() {
                for _ in 0..8 {
                    buffer[idx] = u32::from_be(self.sdmmc.fifor.read().bits());
