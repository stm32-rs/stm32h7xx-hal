...
#[allow(dead_code)]
pub fn display_test<D>(target: &mut D) -> Result<(), D::Error>
where
    D: DrawTarget<Color = Rgb888>,
{
    let style_red =
    ...
    Rectangle::new(Point::new(size.width as i32 - 10, 0), Size::new(10, 10))
        ...
        .draw(target)?;
    Rectangle::new(Point::new(0, size.height as i32 - 10), Size::new(10, 10))
    ...
    let ferris =
        tinybmp::Bmp::from_slice(include_bytes!("../ferris.bmp")).unwrap();
    let ferris = Image::new(
        &ferris,
        Point::new(
            (size.width / 2 - ferris.size().width / 2) as i32,
            (size.height / 2 - ferris.size().height / 2) as i32,
        ),
    );
    ferris.draw(target)?;
    Ok(())
...
#[cfg(feature = "chrono")]
#[allow(dead_code)]
pub fn seven_segment_style(
    color: Rgb888,
) -> eg_seven_segment::SevenSegmentStyle<Rgb888> {
    ...
    D: DrawTarget<Color = Rgb888>,
{
    ...
    Text::new(all_segments, Point::new(x, y), bg_text_style).draw(target)?;
    ...
    let fg_style = PrimitiveStyleBuilder::new().fill_color(fg).build();
    ...
    Circle::new(Point::new(641, y - 40), 10)
        .into_styled(am_style)
        .draw(target)?;
    colored_label("PM", 633, y - 26, Rgb888::RED, target)?;
    Circle::new(Point::new(641, y), 10)
        .into_styled(pm_style)
        .draw(target)?;
    Ok(())
}
